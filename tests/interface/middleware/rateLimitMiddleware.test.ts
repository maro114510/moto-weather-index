...
import type {
  ...
  RateLimitResult,
} from "../../../src/usecase/EnforceRateLimit";
...
function createMockContext(ip: string, headers: Record<string, string> = {}) {
  return {
    req: {
      header: (name: string) =>
        headers[name] || (name === "CF-Connecting-IP" ? ip : undefined),
    },
    json: (
      body: any,
      status: number,
...
describe("rateLimitMiddleware", () => {
  ...
  test("should allow request when rate limit permits", async () => {
    ...
    mockUseCase.setResult("192.168.1.100", {
      allowed: true,
      tokensRemaining: 9,
      retryAfterMs: 0,
    });
  ...
  test("should extract IP from CF-Connecting-IP header", async () => {
    ...
    mockUseCase.setResult("203.0.113.1", {
      allowed: true,
      tokensRemaining: 9,
      retryAfterMs: 0,
    });
  ...
  test("should fallback to X-Forwarded-For if CF-Connecting-IP not present", async () => {
    ...
    mockUseCase.setResult("203.0.113.2", {
      allowed: true,
      tokensRemaining: 9,
      retryAfterMs: 0,
    });
