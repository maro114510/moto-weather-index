...
app.onError((error, c) => {
  ...
  if (error instanceof ZodError) {
    ...
    logErrorResponse(c.req.method, c.req.path, HTTP_STATUS.BAD_REQUEST, startTime, requestContext);
  ...
  if (error instanceof HttpError) {
    ...
    logErrorResponse(c.req.method, c.req.path, statusCode, startTime, requestContext);
  ...
  if (error instanceof TypeError && error.message.includes("fetch")) {
    ...
    logErrorResponse(c.req.method, c.req.path, HTTP_STATUS.SERVICE_UNAVAILABLE, startTime, requestContext);
  ...
  logErrorResponse(c.req.method, c.req.path, HTTP_STATUS.INTERNAL_SERVER_ERROR, startTime, requestContext);
