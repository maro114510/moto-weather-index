 ...
 */
async function verifyHmacSignature(signature: string, secret: string, timestamp: string): Promise<boolean> {
  try {
...
export async function authMiddleware(c: Context, next: Next) {
  ...
  if (!authHeader || !timestamp) {
    return c.json({ error: "Unauthorized", message: "Missing authentication headers" }, HTTP_STATUS.UNAUTHORIZED);
  }

  const secret = process.env.BATCH_SECRET;
  if (!secret) {
    console.error("BATCH_SECRET environment variable not set");
    return c.json({ error: "Internal Server Error", message: "Server configuration error" }, HTTP_STATUS.INTERNAL_SERVER_ERROR);
  }

  const valid = await verifyHmacSignature(authHeader, secret, timestamp);
  if (!valid) {
    return c.json({ error: "Forbidden", message: "Invalid authentication signature" }, HTTP_STATUS.FORBIDDEN);
  }
